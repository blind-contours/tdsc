...
Three influence-function objects must be kept distinct. $D^{\mathrm{np}}_0$ denotes the $2K$-vector of \emph{nonparametric} EIFs \eqref{eq:eif} at $P_0$---the influence function of the unrestricted target $\Psi(P_0)$. $M_n\subseteq\mathcal M$ is the data-adaptive working submodel induced by partially unfreezing $\vartheta$, with working parameter $\Psi_n(P_0)=\Psi(\Pi_n P_0)$ and \emph{working-model} EIF $D^{w}_n$, the $L^2(P_0)$-projection of $D^{\mathrm{np}}_0$ onto the closure of the working score span---the population counterpart of the empirical projection $G\alpha$ that the algorithm computes and whose estimating equations the stopping rule solves. Under ADML there is additionally an oracle submodel with EIF $D^{O}$ to which $D^{w}_n$ converges; the estimators below are asymptotically linear with $D^{\mathrm{np}}_0$ (top-up) or $D^{O}$ (plug-in), and conflating the two is what a careless reading invites---the entire variance story lives in the gap between them.
...
The projection residual $\epsilon_{n,j} = D^{\mathrm{np}}_{0,j} - D^{w}_{n,j}$ stabilizes: $D^{w}_n$ converges in $L^2(P_0)$ to an oracle-model EIF $D^{O}$. \emph{This assumption is not required for Theorem~\ref{thm:main}.} It governs only the plug-in story (Proposition~\ref{prop:plugin}), and the size of $\|\epsilon_n\|$ is a dial, not a defect: a non-vanishing residual means the working parameter genuinely differs from $\Psi(P_0)$ and the plug-in's variance sits strictly below the nonparametric bound; a vanishing residual means working and full targets coincide and the variance advantage disappears asymptotically. The empirical residual $\|D - G\alpha\|/\|D\|$ estimates this quantity.
\end{assumption}
\begin{assumption}[EIF equations solved]\label{a:solved}
$\max_j |\Pn D^{\mathrm{np}}_j| = o_p(n^{-1/2})$ at the final fit (holds exactly, by construction, for the full-residual top-up).
\end{assumption}

\begin{theorem}[Joint asymptotic linearity and efficiency of the top-up estimator]\label{thm:main}
Under Assumptions~\ref{a:id} and~\ref{a:rates} (Assumption~\ref{a:solved} holding by construction; Assumption~\ref{a:span} not required), the cross-fitted TDSC estimator with full residual top-up, $\widehat\Psi^{+}$, satisfies jointly in $\mathbb{R}^{2K}$
