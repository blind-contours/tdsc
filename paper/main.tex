...
\paragraph{Nuisance misspecification and mechanism.} A second study (150 replications per arm; propensity fit by a standalone network throughout so the outcome and weight models can be blinded independently) stresses both arms of double robustness by hiding key covariates from one set of nuisances at a time (Table~\ref{tab:mis}). Three conclusions. \emph{Monotonicity is not the mechanism}: in the well-specified arm the isotonized one-step is indistinguishable from the raw one-step (MSE 0.00154 vs 0.00155), while TDSC's paired MSE advantage is now clearly resolved ($-0.00052$, MC SE $0.00009$)---the gain comes from targeting, not from projecting curves onto the monotone cone. \emph{Point-estimate double robustness holds, with a caveat}: blinding the outcome network to the main confounder and effect modifier explodes the naive plug-in (bias 0.19; RMST bias $-5.8$) while the EIF-corrected estimators remain nearly unbiased; blinding the weight models instead leaves both corrected estimators protected through the hazard arm while the weighting-only IPCW-KM fails (bias 0.076). \emph{Inference is not doubly robust, and TDSC pays first}: under outcome misspecification TDSC's working-model intervals undercover badly (64.6\%; top-up 84.3\%) while the one-step---whose wider intervals track the nonparametric variance---stays at 95.0\%; under weight misspecification both undercover mildly ($\approx$89\%) through the variance estimate. This is the empirical face of the superefficiency caveat of Section~\ref{sec:method}: TDSC's narrower intervals are trustworthy when the initial network is a reasonable fit, and the one-step is the safer instrument when it may not be. Diagnostics flag the failure in-sample: the mean number of coordinates left above tolerance rises from 37 (well-specified; nominal coverage regardless) to 55 (outcome-misspecified), and the strict $\mathrm{sd}/(\sqrt n \log n)$ tolerance is rarely met in full---the line search typically exits first---so we recommend reporting unsolved-coordinate counts and applying the residual top-up as defaults.

\begin{table}[t]
\centering
\caption{Nuisance-misspecification grid, benefit curve ($n=1000$, $K=30$, 150 replications per arm). \emph{outcome mis.}: outcome hazards blinded to $x_1, x_3$; \emph{weights mis.}: propensity and censoring models blinded to $x_1, x_2$.}
\label{tab:mis}
...
outcome mis.   & naive plug-in       & 0.1922 & 0.04110 & ---    & ---   \\
               ...
               & TDSC                & \textbf{0.0119} & 0.01860 & 64.6\% & 0.156 \\
               & TDSC + top-up       & 0.0265 & 0.01325 & 84.3\% & 0.156 \\
\midrule
weights mis.   & IPCW-KM             & 0.0760 & 0.00720 & ---    & ---   \\
