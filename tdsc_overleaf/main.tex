...
This paper closes that gap. TDA already supplies the machinery for many targets at once---its universal path merges per-target directions obtained by projecting stacked EIFs onto the working score span---and we take that machinery as given. What the survival contrast adds is everything around it: a $2K$-dimensional target whose EIFs combine treatment and censoring weights under confounding and dependent censoring; closed-form last-layer gradients that make each merged update a single ridge regression, with no per-sample automatic differentiation; and, running alongside, a deliberately two-part inferential design. The targeted \emph{plug-in} solves the estimating equations that the network's working submodel can actually solve---the projected ones---and earns adaptive-efficiency gains from that restriction; a one-step residual \emph{top-up} then adds back whatever full-EIF bias the restriction leaves behind, recovering a regular, doubly robust estimator of the unrestricted causal target. The two estimators answer different questions, their variances are priced by different influence functions, and much of the paper is devoted to keeping that distinction honest---theoretically, and in simulations designed to make each estimator fail where it should.

\paragraph{Related work.} Neural estimators of treatment-specific survival curves exist---notably SurvITE \citep{curth2021survite}, which targets the conditional counterpart of our estimand with balanced discrete-time hazard networks, and counterfactual time-to-event models \citep{chapfuwa2021}---but they provide point estimates without valid confidence statements; that is the gap we fill. Conversely, survival TMLEs with simultaneous inference exist \citep{vdlgruber2016,cai2020,rytgaard2024}, including universal one-dimensional least-favorable submodels that target an entire survival curve at once; those paths are constructed \emph{analytically}, per estimand and likelihood, whereas TDA's is obtained generically from the architecture's score gradients. TDSC's contribution on this axis is the contrast-specific instantiation of that generic path---the confounded, censored two-arm stack with closed-form last-layer gradients---together with the inference package built on top of it: the working-model versus full-EIF variance split, the doubly robust residual top-up, simultaneous bands, and cross-fitted theory. Causal survival forests \citep{cui2023csf} give honest inference for conditional effects at fixed horizons rather than the full population benefit curve.
...
\item formal guarantees---joint asymptotic linearity, band validity, and a residual top-up that provably restores point-estimate double robustness---for a cross-fitted variant in which sample splitting replaces the Donsker-class conditions of the underlying TDA/ADML theory (resolving, for the top-up, the cross-fitted extension left open in \citealp{li2025tda}), together with a design rule (and a documented failure mode) for \emph{where} the high-dimensional targeting step may be solved (Sections~\ref{sec:method}--\ref{sec:theory});
...
are identified. Our target is the $2K$-vector $\Psi=(S_1(\cdot), S_0(\cdot))$; the \emph{benefit curve} $\beta(t)=S_1(t)-S_0(t)$ and the RMST difference $\Delta_{\mathrm{RMST}}=\sum_{t=1}^{K}\beta(t)$ (bin width one; the contrast in expected bins survived through horizon $K$, i.e.\ in $\E[\min(T-1,K)]$---the conventional $\E[\min(T,K)]$ contrast would be $\sum_{t=1}^{K-1}\beta(t)$) are linear in $\Psi$, so their influence functions follow by linearity. These are the quantities a treatment decision actually turns on: $\beta(t)$ is the absolute survival gain the population would realize at horizon $t$ if everyone were treated versus no one, and $\Delta_{\mathrm{RMST}}$ summarizes it as extra expected survival time. Targeting the whole vector at once, rather than one horizon at a time, is what makes simultaneous inference over the curve possible.

\paragraph{Efficient influence functions.} For each coordinate $S_a(t)$ the EIF at $P$ is the one underlying the randomized-trial TMLE of \citet{moore2009}, extended to the observational setting (estimated propensity in place of randomization; censoring dependent on treatment and covariates):
...
\end{equation}
with $g_1=g$, $g_0=1-g$. Each piece has a role worth naming. The summand is a martingale residual---the within-bin ``surprise'' of an event relative to the fitted hazard---reweighted by the inverse probability of receiving arm $a$ and remaining uncensored through bin $k-1$, so that the observed arm-$a$, still-at-risk subjects stand in for the full population; the factor $S(t\mid a,X)/S(k\mid a,X)$ propagates a hazard error at bin $k$ into its effect on survival at $t$;\footnote{Ratios $S(t)/S(k)$ denote the tail product $\prod_{j=k+1}^{t}\{1-h(j\mid a,x)\}$, well defined even where $S(k)=0$; the implementation additionally floors conditional survival (Appendix~\ref{app:repro}).} and the final terms center the plug-in. Stacking \eqref{eq:eif} over $a$ and $t$ gives the $n\times 2K$ influence matrix $D$ (throughout, $D$ denotes the influence matrix evaluated at the current fitted nuisances; hats distinguish fitted from population objects only where both appear) whose column means $\Pn D_{a,t}$ are the first-order bias terms of the plug-in. The targeting step of Section~\ref{sec:method} drives the \emph{projections} of these means onto the working score span to zero, and the residual top-up removes whatever the projection leaves behind.
...
\draw[arr, blue!70!black] (O) -- (D) node[above, lab, text=blue!70!black] {$D$ (stacked EIFs)};
% projection in-plane
...
\]
giving a direction $\widehat\alpha_{\lambda,j}$ in weight space for each coordinate $j$ and the projected bias terms $d_j = \Pn[G\widehat\alpha_{\lambda,j}]$. The ridge is not decoration: the targeting layers have $p \approx 4{,}000$ parameters against $n=1{,}000$ observations, so the unpenalized projection is ill-posed, and $\lambda$ (scaled to the mean diagonal of $G^\top G$) is statistically active. One deviation from TDA as published: its projection fits an unpenalized intercept per coordinate (equivalently, centers scores and EIFs before the solve); we solve the intercept-free system above. Away from a full-sample optimum the score columns need not be empirically centered, so the two projections can differ by rank-one mean terms; the EIF columns are centered by construction, and near the trained optimum the score means are small, so the discrepancy was negligible here. Writing $\widehat D^{w,\lambda} = G\widehat\alpha_\lambda$ for the resulting ridge estimate of the working-submodel influence functions (Section~\ref{sec:theory} separates it from its population counterpart $D^{w}_n$), the $2K$ per-target directions are merged into one universal direction, $\alpha^* = \sum_j (d_j/\|d\|_2)\, \widehat\alpha_{\lambda,j}$, following the multi-target merge rule of \citet{li2025tda}: each coordinate pulls the weights along its own direction, in proportion to how biased it currently is. The update $\vartheta \leftarrow \vartheta - \gamma\,\alpha^*$ uses a backtracking line search on $\sum_j(\Pn \widehat D^{w,\lambda}_j)^2$ ($\widehat\alpha_\lambda$ held fixed within the search); after each accepted step the hazards, influence matrix, and projection are recomputed, while $g$ and $S_c$ stay fixed.
...
\paragraph{Inference and simultaneous bands.} Two variance prescriptions correspond to the two inferential targets of Section~\ref{sec:theory}: the \emph{top-up} estimator (unrestricted target) uses the full-EIF standard errors $\widehat{\mathrm{sd}}(D_j)/\sqrt n$, while the \emph{plug-in} (working target) uses the working-submodel standard errors $\widehat{\mathrm{sd}}(\widehat D^{w,\lambda}_j)/\sqrt n$, both at the final fit; contrasts inherit influence functions by linearity. For the benefit curve we report a sup-$t$ band over the prespecified grid: with i.i.d.\ standard normal multipliers $\xi_i$ and influence values centered at their empirical mean $\bar D_{\beta,t} = \Pn D_{\beta,t}$, the 95\% quantile $c^*$ of $\max_t |n^{-1}\sum_i \xi_i \{D_{\beta,t}(O_i) - \bar D_{\beta,t}\}|/\widehat{\mathrm{se}}_t$ over bootstrap draws gives $\hat\beta(t) \pm c^*\widehat{\mathrm{se}}_t$ ($c^*$ floored at 1.96, a conservative choice); here $D_{\beta,t}$ denotes the influence values matching the chosen variance prescription---full-EIF for the top-up band, working-model for the plug-in band.

\section{Theory}\label{sec:theory}

TDSC is TDA applied to a $2K$-dimensional pathwise-differentiable target, and its guarantees specialize the working-parameter and original-parameter theorems of \citet{li2025tda} and adaptive debiased machine learning (ADML) \citep{vdl2023adml}. We state the specialization for the \emph{cross-fitted} implementation (Section~\ref{sec:method}; $V$-fold, nuisances trained and targeted out-of-fold, evaluation with exact residual correction on the held-out fold), in which sample splitting replaces those results' Donsker-class conditions---fully so for the top-up estimator (Theorem~\ref{thm:main}, resolving the cross-fitted extension left open in TDA); the plug-in still imports a Donsker-type condition on the training folds, where its projected score equations are solved (Proposition~\ref{prop:plugin}).
...
\item $D^{w}_n$, the \emph{working-model} EIF of the working parameter $\Psi_n(P_0)=\Psi(\Pi_n P_0)$ of the data-adaptive submodel $M_n\subseteq\mathcal M$ induced by partially unfreezing $\vartheta$. Because $M_n$ restricts only the hazards while the covariate marginal remains nonparametric, $D^{w}_n$ retains the centered plug-in component $S_{\Pi_n P_0}(t\mid a,X)-\Psi_{n,j}(P_0)$ intact and projects only the \emph{martingale} component of $D^{\mathrm{np}}_0$ onto the closure of the working score span; the hazard scores are conditionally centered given $(A,X)$, so that span is orthogonal to functions of $X$ alone, and projecting the full EIF onto it would wrongly delete the covariate-marginal variance.\footnote{Strictly, the EIF of $\Psi_n$ in $M_n$ is Hessian-based; the $L^2$ score-span projection replaces the Hessian by the score covariance. Because the loss is a per-bin Bernoulli likelihood, Bartlett's identity makes the two coincide when $M_n$ contains $P_0$ and, under Assumption~\ref{a:span}, in the oracle limit.} Under ADML, $D^{w}_n$ converges to an \emph{oracle}-submodel EIF $D^{O}$, and the plug-in is asymptotically linear with $D^{O}$.
\item $\widehat D^{w,\lambda} = G\widehat\alpha_\lambda$, the ridge-regularized \emph{empirical} counterpart the algorithm actually computes. The algorithm ridge-projects the full influence matrix rather than its martingale component alone; the centering component has empirical mean zero, so the stopping rule is unaffected, and with $p > n$ the near-interpolating fit retains most of that component in $\widehat D^{w,\lambda}$ (compare the working SE 0.032 to the full-EIF SE 0.034 in Table~\ref{tab:main}), at the price of making the empirical working SE's target mildly ridge-dependent.
\end{itemize}
Conflating the first two is what a careless reading invites: the entire variance story lives in the gap between $D^{\mathrm{np}}_0$ and $D^{O}$. For the third object, the assumption splits by use: for the plug-in standard errors, $\|\widehat D^{w,\lambda} - D^{w}_n\|_{P_0} = o_p(1)$ suffices; for the stopping rule to certify the working-model estimating equations at the $o_p(n^{-1/2})$ level Proposition~\ref{prop:plugin} requires, we additionally assume $\Pn(\widehat D^{w,\lambda} - D^{w}_n) = o_p(n^{-1/2})$---the analogue of TDA's approximation condition, whose empirical-process half cross-fitting controls and whose drift half is bounded by the product of the ridge-approximation and initial-estimator rates. A sufficient condition requires the ridge to vanish relative to the \emph{nonzero spectrum} of the empirical score covariance---for example, under a uniform lower bound on its nonzero eigenvalues, $\lambda_n/n \to 0$ suffices; a condition on the mean diagonal alone does not control nearly unidentified score directions. Our simulations fix the relative ridge at $0.01$ as a finite-sample stabilizer; this choice implements, but is not itself covered by, the asymptotic condition (Section~\ref{sec:sims} varies it over two orders of magnitude without material effect).
...
The cross-fitted TDSC \emph{plug-in} (no top-up) is instead an ADML-type estimator of the working parameter $\Psi_n(P_0) = \Psi(\Pi_n P_0)$: under Assumption~\ref{a:span} and the conditions of the working-parameter and original-parameter theorems of \citet{li2025tda}---including their Donsker-type condition, imported here \emph{on the training folds}, where the projected score equations are solved (cross-fitting removes that condition for the top-up but not for the plug-in), and their oracle-bias conditions under which $\Psi_n(P_0)$ may replace $\Psi(P_0)$---it is asymptotically linear with the oracle EIF $D^{O}$. If in addition the oracle submodel contains $P_0$ ($\Pi_0 P_0 = P_0$), then $\mathrm{Var}(D^{O}) \preceq \Sigma_0$ in the Loewner order, with improvement in direction $v$ exactly equal to $\mathrm{Var}(v^\top(D^{\mathrm{np}}_0 - D^{O}))$, strict wherever that limiting residual variance is nonzero (superefficiency, via the likelihood-based oracle-submodel extension of TDA and ADML---our loss is a per-bin Bernoulli likelihood---with the usual price: nonregularity for the unrestricted target under local alternatives outside the oracle model). Its inference must therefore use the \emph{working-model} standard errors $\widehat{\mathrm{sd}}(\widehat D^{w,\lambda}_j)/\sqrt n$; pairing the plug-in with full-EIF standard errors is conservative. Both signatures---lower MSE than Theorem~\ref{thm:main}'s estimator, and over-coverage under full-EIF variance---appear in Section~\ref{sec:sims}.
\end{proposition}
...
Under the conditions of Theorem~\ref{thm:main} and the nondegeneracy $\min_t \mathrm{Var}(D^{\mathrm{np}}_{\beta,t}) > 0$ (immediate for a nondegenerate event hazard), the Gaussian-multiplier estimate of the $95\%$ quantile of $\max_t |\mathbb{G}_t|/\sigma_t$, for $\mathbb{G}$ the limiting Gaussian of Corollary~\ref{cor:contrasts}, is consistent, and the sup-$t$ band covers the benefit curve over the prespecified grid with asymptotic probability at least $0.95$ (the $c^\ast\ge1.96$ floor can only enlarge it). Under Proposition~\ref{prop:plugin} plus consistent estimation of $\mathrm{Cov}(D^{O})$ by the empirical working-model IFs (and the corresponding nondegeneracy), the same multiplier construction applies to the plug-in; in each case the multiplier statistic must be built from the same influence values as the chosen variance prescription.
\end{proposition}

\begin{proposition}[Top-up restores double robustness]\label{prop:dr}
Define the top-up estimator with the \emph{full} residual, $\widehat\Psi^{+} = \widehat\Psi + \Pn \widehat D$; it equals the AIPCW one-step evaluated at the targeted nuisances. Consequently---for the cross-fitted form, under Assumption~\ref{a:id} and convergence of the estimated nuisances to \emph{some} limits respecting the bounds of Assumption~\ref{a:id} (enforced in practice by the truncation of Appendix~\ref{app:repro})---$\widehat\Psi^{+}$ is consistent for $\Psi(P_0)$ if either the hazard limit equals $h_0$ or the weight limits equal $(g_0, S_{c,0})$ jointly, regardless of whether targeting converged. The no-split form additionally requires a uniform law of large numbers for the estimated-EIF class, which cross-fitting renders unnecessary.
\end{proposition}
...
outcome mis. & naive plug-in            & 0.1904 & 0.04028 & --- & --- & --- \\
             ...
             & TDSC plug-in (working SE)  & 0.0915 & 0.01004 & 33.1\% & 0.150 & 15.0\% \\
...
gives $R_{2,j}$ as a sum over $k\le t$ of integrals of $\{h_0(k) - \hat h(k)\}$ against $\{\hat g_a\hat S_c(k-1) - g_{0,a} S_{c,0}(k-1)\}/\{\hat g_a \hat S_c(k-1)\}$-type differences; under the positivity bounds of Assumption~\ref{a:id}, Cauchy--Schwarz gives $|R_{2,j}| \lesssim \|\hat h - h_0\|_{P_0}(\|\hat g - g_0\|_{P_0} + \|\hat S_c - S_{c,0}\|_{P_0}) = o_p(n^{-1/2})$ by Assumption~\ref{a:rates}, applied to the targeted per-fold hazards (this display also proves Proposition~\ref{prop:dr}: $R_2$ vanishes if either factor's limit is correct). For the empirical-process term, cross-fitting makes the fold estimate $\widehat D_{v,j}$ independent of fold $v$, so conditional on the training folds, $(P_{n,v} - P_0)(\widehat D_{v,j} - D_{0,j}) = O_p(n^{-1/2}\|\widehat D_{v,j} - D_{0,j}\|_{P_0}) = o_p(n^{-1/2})$ by Chebyshev whenever $\|\widehat D_{v,j} - D_{0,j}\|_{P_0} = o_p(1)$, which follows from the individual nuisance consistency in Assumption~\ref{a:rates}; summing the $V$ folds gives the pooled statement and the coordinate-wise expansion, and joint convergence follows from the multivariate CLT applied to the stacked $D_0$. For the variance estimator: the positivity bounds of Assumption~\ref{a:id} give a bounded envelope for the estimated EIFs, which with $\|\widehat D_j - D_{0,j}\|_{P_0} = o_p(1)$ and out-of-fold evaluation yields $\Pn[\widehat D\widehat D^\top] \to_p \Sigma_0$, so the reported standard errors and the band's $\widehat{\mathrm{se}}_t$ are consistent. Proposition~\ref{prop:plugin} follows the working-parameter and original-parameter arguments of \citet{li2025tda} with two modifications: the linear expansion is evaluated out-of-fold, and their Donsker-type condition is imported on the training folds, where the projected score equations are solved. Superefficiency follows from the likelihood-based oracle-submodel extension of those results (the loss here is a per-bin Bernoulli likelihood) together with \citet{vdl2023adml}.

\paragraph{Corollary~\ref{cor:contrasts} and Proposition~\ref{prop:band}.} Both maps are bounded linear in $\Psi$, so asymptotic linearity transfers with the transformed influence functions. Given joint normality with consistently estimated $\Sigma_0$ (IF second moments; see the Theorem~\ref{thm:main} sketch) and the nondegeneracy $\min_t \sigma_t > 0$, the conditional multiplier process $n^{-1/2}\sum_i \xi_i \{\widehat D_{\beta,t}(O_i) - \bar D_{\beta,t}\}/\widehat{\mathrm{se}}_t$---the studentized, centered statistic the algorithm computes; the centering constant is $O_p(n^{-1/2})$, hence asymptotically negligible---converges weakly to the standardized Gaussian limit (conditional multiplier CLT), whose sup-distribution is continuous and strictly increasing at its 0.95 quantile, so the estimated sup-quantile is consistent; the $c^*\ge1.96$ floor only enlarges the band.
...
\newblock In \emph{Proceedings of the Conference on Health, Inference, and Learning (CHIL)}, pages 133--145, 2021.
...
\bibitem[van~der Laan and Gruber(2016)]{vdlgruber2016}
M.~J. van~der Laan and S.~Gruber.
\newblock One-step targeted minimum loss-based estimation based on universal least favorable one-dimensional submodels.
\newblock \emph{The International Journal of Biostatistics}, 12(1):351--378, 2016.
\newblock \url{https://doi.org/10.1515/ijb-2015-0054}.
